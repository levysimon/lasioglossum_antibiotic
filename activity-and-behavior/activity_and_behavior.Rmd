...
model_lmm <- lmer(average.speed ~ antibiotic * reinoculation + (1 | year:arena), data = df_move)
...
```{r}
model_lmm_add <- lmer(average.speed ~ antibiotic + reinoculation + (1 | year:arena), data = df_move)
...
model_lmm_red <- lmer(average.speed ~ antibiotic * reinoculation + (1 | year:arena), data = df_move_red)
...
model_lmm_add <- lmer(average.speed ~ antibiotic + reinoculation + (1 | year:arena), data = df_move_red)
simulateResiduals(model_lmm_add) |> plot()
testDispersion(model_lmm_add)
testResiduals(model_lmm_add)
...
p_model <- ggplot() +
  ...
  geom_point(data = df_move, aes(x = antibiotic, y = average.speed, fill = reinoculation), shape = 21, colour = "black", stroke = 0.3, size = 3.5, alpha = 0.9, position = position_jitterdodge(jitter.width = 0.37, dodge.width = 0.8, seed = 32)) +
...
# BETADIASPER RESULT - Test significance of dispersion
permu_ant <- permutest(dispersion_mod, permutations = 1000)
permu_ant
# C. PERMANOVA (Adonis2)
# This tests: "Are the centers of the groups significantly different?"
...
# BETADIASPER RESULT - Test significance of dispersion
permu_reino <- permutest(dispersion_mod, permutations = 10000)
permu_reino
