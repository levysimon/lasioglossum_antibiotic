...
``` r
mean_bump <- tapply(df_behavior$nb.of.bump, df_behavior$treatment, mean, na.rm = TRUE)
sd_bump <- tapply(df_behavior$nb.of.bump, df_behavior$treatment, sd, na.rm = TRUE)

result_bump <- data.frame(
  Mean = mean_bump,
  Std_dev = sd_bump
)

mean_avoidance <- tapply(df_behavior$nb.of.avoidance, df_behavior$treatment, mean, na.rm = TRUE)
sd_avoidance <- tapply(df_behavior$nb.of.avoidance, df_behavior$treatment, sd, na.rm = TRUE)

result_avoidance <- data.frame(
  Mean = mean_avoidance,
  Std_dev = sd_avoidance
)

mean_head_to_head <- tapply(df_behavior$nb.of.head.to.head, df_behavior$treatment, mean, na.rm = TRUE)
sd_head_to_head <- tapply(df_behavior$nb.of.head.to.head, df_behavior$treatment, sd, na.rm = TRUE)

resultats_head_to_head <- data.frame(
  Mean = mean_head_to_head,
  Std_dev = sd_head_to_head
)

mean_touch <- tapply(df_behavior$nb.of.touch, df_behavior$treatment, mean, na.rm = TRUE)
sd_touch <- tapply(df_behavior$nb.of.touch, df_behavior$treatment, sd, na.rm = TRUE)

result_touch <- data.frame(
  Mean = mean_touch,
  Std_dev = sd_touch
)

formating_stats <- function(mean, sd) {
  paste0(round(mean, 1), " \u00B1 ", round(sd, 1)) # \u00B1 is the symbol +/-
}

table_publication <- data.frame(
  Category = rownames(result_bump), 
  stringsAsFactors = FALSE
)

table_publication$`Random bump` <- formating_stats(
  mean_bump, 
  sd_bump
)

table_publication$`Short antennal touch` <- formating_stats(
  mean_touch, 
  sd_touch
)

table_publication$`Head-to-head interaction` <- formating_stats(
  mean_head_to_head, 
  sd_head_to_head
)

table_publication$Avoidance <- formating_stats(
  mean_avoidance, 
  sd_avoidance
)

colnames(table_publication)[1] <- "Treatment"
...
write.csv(table_publication, file = output_file_path, row.names = FALSE)
```
...
``` r
mean_Mean_dist <- tapply(df_interind$mean.dist, df_interind$treatment, mean, na.rm = TRUE)
sd_Mean_dist <- tapply(df_interind$mean.dist, df_interind$treatment, sd, na.rm = TRUE)

result_mean_dist <- data.frame(
  Mean_cm = mean_Mean_dist,
  Sd_cm = sd_Mean_dist
)
...
write.csv(result_mean_dist, here::here("activity-and-behavior", "result", "inter_individual_dist", "table_dist_interind.csv"), row.names = TRUE)
```
...
``` r
mean_speed <- tapply(df_move$average.speed, df_move$treatment, mean, na.rm = TRUE)
sd_speed <- tapply(df_move$average.speed, df_move$treatment, sd, na.rm = TRUE)

result_speed <- data.frame(
  Mean = mean_speed,
  Std_dev = sd_speed
)

mean_prop <- tapply(df_move$prop.time.moving, df_move$treatment, mean, na.rm = TRUE)
sd_prop <- tapply(df_move$prop.time.moving, df_move$treatment, sd, na.rm = TRUE)

result_prop <- data.frame(
  Mean = mean_prop,
  Std_dev = sd_prop
)
...
write.csv(result_speed, here::here("activity-and-behavior", "result", "activity", "avg_speed_table.csv"), row.names = TRUE)
write.csv(result_prop, here::here("activity-and-behavior", "result", "activity", "prop_of_time_moving_table.csv"), row.names = TRUE)
```
...
## Extract and save PCA loadings (variable contributions to PC1/PC2)

``` r
# Extract variable coordinates/loadings from the dimensio PCA object 

var_coords <- get_coordinates(X, margin = 2, principal = TRUE)
loadings_df <- as.data.frame(var_coords)
loadings_df$Variable <- rownames(loadings_df)

# Keep only the axes of interest (PC1 = F1, PC2 = F2 in dimensio's convention)
loadings_df <- loadings_df %>%
  dplyr::select(Variable, F1, F2) %>%
  dplyr::rename(PC1 = F1, PC2 = F2) %>%
  dplyr::arrange(desc(abs(PC1)))

# --- 2. Save the loadings table ---
results_dir_pca <- here::here("activity-and-behavior", "result", "PCA_on_7metrics")

dir.create(
  path = results_dir_pca, 
  showWarnings = FALSE, 
  recursive = TRUE
)
...
write.csv(
  loadings_df,
  file.path(results_dir_pca, "PCA_7metrics_loadings.csv"),
  row.names = FALSE
)

# --- 3. Nice loadings plot for PC1 (main axis of interest) ---
p_loadings_pc1 <- ggplot(loadings_df, aes(x = reorder(Variable, PC1), y = PC1, fill = PC1 > 0)) +
  geom_col(width = 0.45, alpha = 1) +
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#4F77B4", "FALSE" = "#FFBF00"), guide = "none") +
  scale_y_continuous(
...
print(p_loadings_pc1)
```

``` r
ggsave(
  filename = file.path(results_dir_pca, "PCA_7metrics_PC1_loadings.pdf"),
  plot = p_loadings_pc1,
  width = 7, height = 5
)
```
...
p1 <- ggplot() +
  ...
  # 2. LMM  Result : Horizontal line for predicted mean
  geom_errorbar(
    data = emm_df,
    aes(x = antibiotic, ymin = emmean, ymax = emmean, group = reinoculation),
    width = 0.5, 
